...
if (!localStorage.getItem("travelRequests"))
  localStorage.setItem("travelRequests", JSON.stringify([]));
if (!localStorage.getItem("addedExpenses"))
  localStorage.setItem("addedExpenses", JSON.stringify([]));
if (!localStorage.getItem("advanceRequests"))
  localStorage.setItem("advanceRequests", JSON.stringify([]));
if (!localStorage.getItem("postSettlements"))
  localStorage.setItem("postSettlements", JSON.stringify([]));
...
function renderData(tab) {
  if (tab === "dashboard") {
    ...
    dash.innerHTML = `<p>Pending Travels: ${
  ...
  if (tab === "travel") {
    ...
    if (travels.length === 0) list.innerHTML = "<p>No travel requests</p>";
      ...
      list.innerHTML = travels
        .map((t, i) => `<div>${i + 1}. ${t.purpose} - ${t.status}</div>`)
        .join("");
  }
  if (tab === "expenses") {
    ...
    if (expenses.length === 0) list.innerHTML = "<p>No expenses</p>";
      ...
      list.innerHTML = expenses
        .map(
          (e, i) =>
            `<div>${i + 1}. ${e.category} - ${e.amount} - ${e.status}</div>`
        )
        .join("");
  }
  if (tab === "advance") {
    ...
    const eligibleAdv = adv.filter(
      (a) => a.eligible && a.status === "Approved" && !a.settled
    );
    if (eligibleAdv.length === 0)
      list.innerHTML = "<p>No advance pending settlement</p>";
    else
      list.innerHTML = eligibleAdv
        .map(
          (a, i) =>
            `<div>${i + 1}. ${a.travelPurpose} - ${
              a.amount
            } - Pending Settlement</div>`
        )
        .join("");
  }
}
...
function newTravel() {
  ...
  const advance =
    type.toLowerCase() == "eligible"
      ? prompt("Advance Required? Yes/No", "Yes")
      : "No";
  const advanceAmount =
    advance.toLowerCase() == "yes" ? prompt("Enter advance amount:", 0) : 0;
  const travels = JSON.parse(localStorage.getItem("travelRequests"));
  travels.push({
    purpose,
    status: "Pending",
...
function newExpense() {
  const category = prompt(
    "Enter category (Petrol/Fuel, Vehicle Maintenance, etc):"
  );
  if (!category) return;
...
function updateBadges() {
  ...
  document.getElementById("dashboardBadge").textContent =
    travelData.filter((t) => t.status === "Pending").length +
    expenseData.filter((e) => e.status === "Pending").length;
  document.getElementById("travelBadge").textContent = travelData.filter(
    (t) => t.status === "Pending"
  ).length;
  document.getElementById("expensesBadge").textContent = expenseData.filter(
    (e) => e.status === "Pending"
  ).length;
  document.getElementById("settleBadge").textContent = settleData.filter(
    (s) => s.status === "Pending"
  ).length;
  const advanceCount = advanceData.filter(
    (a) => a.eligible && a.status === "Approved" && !a.settled
  ).length;
